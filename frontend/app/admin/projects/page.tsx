...
function ProjectsPageContent() {
  ...
  const pageTab = (searchParams.get("tab") as "hero" | "projects") || "projects";
  ...
  async function fetchHeroSettings() {
    try {
      const res = await getProjectPageData();
      if (res.hero) {
  ...
  async function handleHeroSave(e: React.FormEvent) {
    ...
    try {
      ...
      await updateProjectPage(fd);
      toast.success("Project page hero updated successfully!");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-neutral-900 font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900">Project Setup — {tabLabel[pageTab]}</h1>
          <p className="text-sm text-neutral-500 mt-1">Configure the hero banner and manage project items.</p>
        </div>
      ...
      <div>
        ...
        <div className="space-y-6">
          {pageTab === "hero" ? (
            /* ── Project Page Hero Banner Editor ─────────────── */
            <div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden animate-in zoom-in-95 duration-300">
              <div className="px-8 py-5 border-b border-neutral-100 bg-neutral-50/30">
                ...
                <p className="text-xs text-neutral-400 mt-1">This image and title appear at the top of the user-facing Projects page.</p>
              </div>
          ...
          <div className="overflow-x-auto">
            ...
            <div className="px-6 py-4 bg-neutral-50/30 border-t border-neutral-100 flex items-center justify-between">
              <div className="text-sm text-neutral-600 font-medium">
                Showing {filteredProjects.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1}-{Math.min(currentPage * ITEMS_PER_PAGE, filteredProjects.length)} of {filteredProjects.length}
              </div>
              <div className="flex items-center gap-2">
                <button
